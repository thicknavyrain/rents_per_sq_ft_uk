...
def _(pd):
    ...
    #Filter by columns for house identifier, number of bedrooms, tenancy type and dwelling type 
...
def _(pd):
    ...
    # Filter by columns for house identifier, dwelling type (3 columns), floor space (two differently defined use cases, see data dictionary) and EPC rating
...
def _(pd):
    ...
    #Filter by columns for home identifier, Region in England, type of tenure (2 columns) 
...
def _(EHS_general_df, EHS_interview_df, EHS_physical_df, pd):
    ...
    # Function which implements various regex patterns to tidy strings into float-type data to enable calculation of floorspace averages
    ...
    merged_df.to_csv("EHS_analysis_outputs/EHS_bedrooms_floor.csv", index=False, float_format="%.2f")
